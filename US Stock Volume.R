...
full_data <- vol_data
train_ratio <- 0.5
train_size <- floor(train_ratio * nrow(vol_data))
train_data <- vol_data[1:train_size, ]
...
compute_tpm <- function(states, prob) {
  uniq_states <- sort(unique(states))
...
###################################### COMPUTE TPM ##############################################
TPM_full <- compute_tpm(full_data$State, TRUE)
TPM_train <- compute_tpm(train_data$State, TRUE)
TPM_count_train <- compute_tpm(train_data$State, FALSE)
###################################### OUTPUT TPM ##############################################
cat("\n--- Full Dataset TPM ---\n")
print(TPM_full)
...
###################################### PLOT VOLUME GRAPH ##############################################
ggplot() +
  geom_line(data = full_data, aes(x = Date, y = Volume), color = "brown") +
  geom_line(data = train_data, aes(x = Date, y = Volume), color = "blue") +
  labs(title = "",
       x = "Year", y = "Stock Volume") +
  theme_minimal()
##################################### FIT ON TRAINING ########################################################
...
########################## Calculate MLE and GSBP #########################
  ...
  dim_sample <- nrow(TPM_count_train)
  n[1:dim_sample, 1:dim_sample] <- TPM_count_train
...
################################## ##############################################
 ##################################### GRAPH of TEST #####################################
  ...
  ###################################### SPLIT TEST DATA ##############################################
  test_data <- vol_data[(train_size + 1):nrow(vol_data), ]
  ...
  initial_state <- tail(train_data$State, 1)
...
###################################### FILTER TEST PERIOD DATA ##############################################
  ...
  test_states <- floor(test_data$Volume)  # Convert to integer states
  
  # Update test_plot_data for Actual
  test_plot_data$Volume[test_plot_data$Type == "Actual"] <- test_states
###################################### PLOT TEST PERIOD ONLY ##############################################
  library(ggplot2)
  
  ggplot(test_plot_data, aes(x = Date, y = Volume, color = Type)) +
